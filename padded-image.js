...
program.args.forEach(async imgPath=>{
  ...
  let brightnessParams = _.range(-20,20,8).map(v=>v/100)
  let contrastParams = _.range(-20,20,8).map(v=>v/100)

  _.product(gaussianParams,brightnessParams,contrastParams).forEach(([a,b,c],i)=>{
    setTimeout(()=>{
      img.clone().gaussian(a).brightness(b).contrast(c).writeAsync(`data/${imgName}-${i}.png`)
    });
  })
})
